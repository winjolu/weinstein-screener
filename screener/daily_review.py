...
def format_review(result):
    ...
    if result["urgent"]:
        ...
        for hit in result["urgent"]:
            lines.append(f"  {hit['ticker']}: traded down to {hit['low']:.2f}, "
                         f"through the stop at {hit['stop']:.2f} "
                         f"(week of {hit['date']})")
        lines.append("")
    ...
    if not result["weekly_bar_closed"]:
        ...
        lines.append("Candidates, sized against the account:")
    ...
    lines.append("")
    lines.append("Check each of these against the charts before acting on")
    lines.append("any of it, then record what was actually done. What is declined")
    lines.append("is as useful to the record as what is taken.")
    return "\n".join(lines)
